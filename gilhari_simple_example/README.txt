...
- The compile.cmd (compile.sh) file is used to compile the container domain model classes. The sources.txt file contains a list of the source (.java) files for the underlying shell (container) classes. The compilation of the domain model classes is done for a target JDK version of 1.8 which is compatible with the current version of Gilhari.
...
{"jdx_orm_spec_file": "./config/gilhari_simple_example.jdx",
 "jdbc_driver_path": "/node/node_modules/jdxnode/external_libs/sqlite-jdbc-3.8.11.2.jar",
 "jdx_debug_level": 3,
